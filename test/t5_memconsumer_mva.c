...
void fill_data(unsigned long long *mem, unsigned long end_idx, int re)
{
...
void check_data(unsigned long long *mem, unsigned long end_idx, int re, int subre)
{
	unsigned long long curr = 0;
	...
	while (curr < end_idx) {
		...
		if (curr % (SHOW_UNIT*ITER_UNIT) == 0) {
			...
			gettimeofday(&now, NULL);
#endif
			printf("[%04d-%d]checked: %10lldMB/%10lldMB (%5lldGB/%5lldGB)"
					"time: %ld.%03lds\n", re, subre,
...
#define ALLOC_MMAP
unsigned long long *mem_alloc(unsigned long long size)
{
...
int main(int argc, char **argv) 
{
	...
	for (re = 0; re < REPEAT; re++) {
		end_idx = MEMSIZE / 2 / sizeof(unsigned long long);

		memA = mem_alloc(MEMSIZE / 2);
		fill_data(memA, end_idx, re);

		memB = mem_alloc(MEMSIZE / 2);
		fill_data(memB, end_idx, re);

		for (i = 0; i < CHECK_REPEAT; i++) {
			check_data(memA, end_idx, re, i);
			check_data(memB, end_idx, re, i);
		}

		mem_free(memA, MEMSIZE / 2);
		mem_free(memB, MEMSIZE / 2);
	}
