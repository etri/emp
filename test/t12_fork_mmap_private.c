...
int main(int argc, char **argv) 
{
	...
	if (pid == 0) {
		...
		total = 0;
		checked = 0;
		printf("\n2. value WRITTEN BY PARENT, CHILD READS WELL\n");
		fprintf(stderr, "this is a child process pid:%d\n", getpid());
		check_data_same(mem, label, 2, 0, &checked, &total, 0, end_idx, 0, 50ul, SHOW_UNIT);
	...
	total = 0;
	checked = 0;
	printf("\n5. PARENT READS the value PREVIOUSLY WRITTEN by the PARENT\n");
	fprintf(stderr, "this is a parent process pid:%d\n", getpid());
	check_data_same(mem, label, 4, 0, &checked, &total, 0, end_idx, 0, 50ul, SHOW_UNIT);

	total = 0;
	checked = 0;
	printf("\n6. PARENT WRITE the different value, PARENT CHECK it again\n");
	fprintf(stderr, "this is a parent process pid:%d\n", getpid());
	fill_data(mem, end_idx, label, 5, 150ul);
	check_data_same(mem, label, 5, 0, &checked, &total, 0, end_idx, 0, 150ul, SHOW_UNIT);
