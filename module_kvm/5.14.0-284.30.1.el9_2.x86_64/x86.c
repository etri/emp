...
#include "kvm_emp.h"
extern struct emp_mod emp_mod;

static int emp_mark_free_pages(struct kvm *kvm, gfn_t gfn, long num_pages)
{
	if (!emp_is_installed(kvm))
		return -EINVAL;

	if (emp_mod.mark_free_pages == NULL)
		return -EPERM;

	return emp_mod.mark_free_pages(kvm, gfn, num_pages);
}
#endif
...
#ifdef CONFIG_KVM_EMP
	case KVM_HC_EMP_MARK_FREE_PAGES:
		ret = emp_mark_free_pages(vcpu->kvm, (gfn_t) a0, (long) a1);
		break;
#endif
	default:
		ret = -KVM_ENOSYS;
...
static void memslot_rmap_free(struct kvm_memory_slot *slot)
{
	...
	for (i = 0; i < KVM_NR_PAGE_SIZES; ++i) {
		...
		int lpages = __kvm_mmu_slot_lpages(slot, slot->npages, i + 1);
		int sz = sizeof(*slot->arch.rmap[0]);
		printk(KERN_ERR "[%s] id: %d as_id: %d npages: 0x%lx level: %d "
				"lpages: 0x%x sz: 0x%x alloc: 0x%lxB (%ldMB)\n",
				__func__, slot->id, slot->as_id, slot->npages,
				i + 1, lpages, sz,
				((size_t) lpages) *  sz,
				(((size_t) lpages) * sz) >> 20);
#endif
...
int memslot_rmap_alloc(struct kvm_memory_slot *slot, unsigned long npages)
{
	...
	for (i = 0; i < KVM_NR_PAGE_SIZES; ++i) {
		...
		kvm_alloc_profile(KVM_ALLOC_MEMSLOT_RMAP, ((size_t) lpages) * ((size_t) sz));
#ifdef CONFIG_KVM_ALLOC_PROFILE
		printk(KERN_ERR "[%s] id: %d as_id: %d npages: 0x%lx level: %d "
				"lpages: 0x%x sz: 0x%x alloc: 0x%lxB (%ldMB)\n",
				__func__, slot->id, slot->as_id, slot->npages,
				...
				(((size_t) lpages) * sz) >> 20);
#endif
#ifdef CONFIG_KVM_PREFER_DCPMM
		slot->arch.rmap[i] = __vmalloc_node(((size_t) lpages) * sz, 1,
				__GFP_MOVABLE | GFP_KERNEL_ACCOUNT | __GFP_ZERO,
				2 + slot->as_id, __builtin_return_address(0));
#else
				      ...
				      struct kvm_memory_slot *slot)
{
	...
	for (i = 1; i < KVM_NR_PAGE_SIZES; ++i) {
		...
		kvm_alloc_profile(KVM_ALLOC_MEMSLOT_META, ((size_t) lpages) * ((size_t) sizeof(*linfo)));
