...
/*
 * trace point for recording emp events
 * */
TRACE_EVENT(emp_remote_events,
	TP_PROTO(unsigned long gpa, int op),
	TP_ARGS(gpa, op),
	TP_STRUCT__entry(
	...
	TP_printk("emp_remote_event gpa %lx op %d",
		__entry->gpa, __entry->op)
);

TRACE_EVENT(emp_inactive_events,
        TP_PROTO(unsigned long gpa, int op),
        TP_ARGS(gpa, op),
        TP_STRUCT__entry(
                __field(        unsigned long,  gpa     )
        ...
        TP_printk("emp_inactive_event gpa %lx op %d",
                __entry->gpa, __entry->op)
);
#endif /* CONFIG_KVM_EMP */
