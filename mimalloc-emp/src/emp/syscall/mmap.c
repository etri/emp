...
#ifdef EMP_DEBUG
		fprintf(stderr, "%s(): addr=0x%lx, length=0x%lx, prot=%d, flags=%d, fd=%d, offset=%zu, ret=0x%lx\n", __func__, (unsigned long)addr, length, prot, emp_flags, empfd, offset, (unsigned long)ret);
#endif
	}
	else {
		ret = __syscall(SYS_mmap, addr, length, prot, flags, fd, offset);
#ifdef EMP_DEBUG
		fprintf(stderr, "%s(): addr=0x%lx, length=0x%lx, prot=%d, flags=%d, fd=%d, offset=%zu, ret=0x%lx\n", __func__, (unsigned long)addr, length, prot, flags, fd, offset, (unsigned long)ret);
#endif
	}
#endif
